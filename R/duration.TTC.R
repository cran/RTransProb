...
duration.TTC <- function(lstCnt,lstFirmYears){
  ...
  # Initial matrices
  if (gHorizon <= 1){
    genAnnual  <- rep(list(list()), length(lstCnt))   #rep(list(list()), gHorizon+1)  #
    transAnnual  <- rep(list(list()), length(lstCnt))  #rep(list(list()), gHorizon+1)
  } else {
    genAnnual  <- rep(list(list()), length(lstCnt))   #rep(list(list()), gHorizon)  #
    transAnnual  <- rep(list(list()), length(lstCnt)) #rep(list(list()), gHorizon)
  }
  genAverage  <- list()
  transAverage  <- list()
  ...
  for (i in 1:nYear){
    ...
    for (l in 1:length(transCount[[i]])){
      ...
      if(i==1 && l==1){
        ...
        genRawNum <- genRawNum + ((genWeight[i]/gHorizon)/scaler[i])*(as.matrix(as.data.frame(transCount[[i]][l])))
        # Section 1.b
        # (scale the weights of the firm years, apply that scaler to the firm-years  and compound the results)- genRawDen
        genRawDen <- genRawDen + ((genWeight[i]/gHorizon)/scaler[i])*pracma::repmat(matrix(firmYears[i,],nStates),1,nStates)
      ...
      if(i==1 && l==1){
        ...
        genAverage <-  genAverage + (genWeight[i]/gHorizon)*genAnnual[[i]][[l]]
      }
      ...
      if(i==1 && l==1){
        ...
        transAverage <- transAverage + (genWeight[[i]]/gHorizon)*transAnnual[[i]][[l]]
  ...
  outPut  <- list(PTMLW=transAverage,                        #Construct TTC transition matrix using the average of all of the weighted transition matrices for each time-step(Scaling is performed at the transition matrix level for each time-step)
                  PGMLW=transAverageGen,                     #Construct TTC transition matrix using the average of all of the weighted Generator matices for each time-step(Scaling is performed at the generator matrix level for each time-step)
                  CLW=transRaw,                              #Construct TTC transition matrix from scaled and weighted counts data (transitions and firm years)
                  UUPTM=transAnnual,                         #Construction of unscaled and unweighted transition matrices for each time-step from unscaled and unweighted generator matrices for each time-step
                  WGM=genAverage,                            #weighted generators for each time-step
                  SWT = genRawNum,                           #aggregate scaled and weighted transitions
                  SWFY = genRawDen)                          #aggregate scaled and weighted firm years        
