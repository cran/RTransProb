...
cfdates <- function(sdates,edates,snapshots){
  ...
  if(snapshots==1){
    ...
    snapshotDates <- seq(ISOdate(sdates_yrs, sdates_mns,sdates_dys), ISOdate(edates_yrs, edates_mns,edates_dys), by = "quarter")
    if(length(snapshotDates)<2){
      snapshotDates[2] <- ISOdate(edates_yrs, edates_mns,edates_dys)
    }
  } else if(snapshots==12){
    snapshotDates <- as.Date(chron::seq.dates(format(sdates,"%m/%d/%Y"),format(edates,"%m/%d/%Y"),by="months"),"%Y-%m-%d")
  } else if(snapshots==54){
    snapshotDates <- seq(ISOdate(sdates_yrs, sdates_mns,sdates_dys), ISOdate(edates_yrs, edates_mns,edates_dys), by = "week")
